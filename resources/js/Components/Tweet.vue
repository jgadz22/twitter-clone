    ...
    <div class="p-2 w-full">
        <div class="font-extrabold flex items-center justify-between mt-0.5 mb-1.5">
            <div class="flex items-center">
                <div>{{ tweet.name }}</div>
                <span class="text-gray-500 pl-2 font-[300] text-[15px]">{{ tweet.handle }}</span>
            </div>
            <div class="rounded-full cursor-pointer relative hover:bg-gray-800">
                <button type="button" class="block p-2">
                    <DotsHorizontal @click="openOptions = !openOptions" />
                </button>

                <div v-if="openOptions" class="absolute mt-1 p-3 right-0 w-[300px] bg-black border border-gray-700 rounden-lg shadow-lg">
